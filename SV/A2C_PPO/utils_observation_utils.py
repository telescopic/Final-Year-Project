...
def norm_obs_clip(obs,
                  clip_min=-1,
                  clip_max=1,
                  fixed_radius=0,
                  normalize_to_range=False):
    """
...
def _split_node_into_feature_groups(
        node) -> (np.ndarray, np.ndarray, np.ndarray):
    data = np.zeros(6)
    distance = np.zeros(1)
...
def _split_subtree_into_feature_groups(
    ...
    if node == -np.inf:
        ...
        return [-np.inf] * num_remaining_nodes * 6, [
            -np.inf
        ] * num_remaining_nodes, [-np.inf] * num_remaining_nodes * 4
...
def split_tree_into_feature_groups(
        tree, max_tree_depth: int) -> (np.ndarray, np.ndarray, np.ndarray):
    """
...
def normalize_observation(observation, tree_depth: int, observation_radius=0):
    ...
    normalized_obs = np.concatenate((np.concatenate((data, distance)),
                                     agent_data))
