...
agent = Model(gamma=0.99,epsilon=1.0,lr=1e-4,
                    ...
                    input_dims=[state_size], mem_size=25000, mem_alpha=0.6,
                    batch_size=32, beta=0.4, beta_max=1.0, beta_increment=6e-5,
                    eps_min=0.06, replace=1000, eps_dec=1e-5,
...
for episode_count in range(n_episodes):
    ...
    # print(agent.memory.terminal_memory, done)
    temp = list(filter(None, agent.memory.terminal_memory))
    temp = temp[-2*env_params['n_agents']:-env_params['n_agents']]
    print(temp)
    scores.append(score)
