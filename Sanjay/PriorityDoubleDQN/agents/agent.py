...
class Model():
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims,
                mem_size, mem_alpha, batch_size, beta, beta_max, beta_increment, eps_min=0.01, eps_dec=5e-7, 
        ...
        print(self.memory.action_memory)
    ...
    def sample_memory(self):
        state, action, reward, new_state, done, importance = self.memory.sample_buffer(self.batch_size, self.beta)
        ...
        importance = T.tensor(importance).to(self.q_eval.device)

        return states, actions, rewards, new_states, dones, importance
    ...
    def learn(self):
        ...
        #print("LEARNING")
        states, actions, rewards, next_states, dones, importance = self.sample_memory()
        #print(states)
        ...
        errors = self.q_eval.loss(q_target, q_pred).to(self.q_eval.device)

        loss = errors * importance
        loss = loss.mean()
        ...
        self.memory.update_priorities(indices, errors.cpu().detach().numpy(), offset=0.1)
